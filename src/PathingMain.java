...
public class PathingMain extends PApplet
{
   ...
   */
   private boolean moveOnce(Point pos, GridValues[][] grid, List<Point> path)
   {
      ...
      //make point go right until cant go right anymore
      //make point go down, if it cant - go left, if it cant - go up

      //test if this is a valid grid cell
      boolean found; //make a found argument that is true when grid goal is found, otherwise false.
      ...
      if (!withinBounds(pos, grid)  || grid[pos.y][pos.x] == GridValues.OBSTACLE || grid[pos.y][pos.x] == GridValues.SEARCHED) {
      ...
      else if (grid[pos.y][pos.x] == GridValues.GOAL) {
         found = true;
      }
         ...
         found = moveOnce(right, grid, path) || moveOnce(left, grid, path) || moveOnce(down, grid, path) || moveOnce(up, grid, path);
      }
